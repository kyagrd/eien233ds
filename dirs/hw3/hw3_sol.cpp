...
list* append(list* l1, list* l2) {
	if (l1 == NULL) {
		...
		// 길이 1 이상인 리스트
		list* p = append(l1->next, l2); // 맨앞 뺀 나머지 복사
		return new list({ l1->data, p });
	}
}
...
list* preorder(tree* t) {
	if (t == NULL) { return NULL;
		...
		// cout << t->data << " ";
		list* l1 = preorder(t->left);
		list* l2 = preorder(t->right);
		return new list({ t->data, append(l1,l2) });
	}
}
...
list* postorder(tree* t) {
	if (t == NULL) {
		...
		list* l1 = postorder(t->left);
		list* l2 = postorder(t->right);
		// cout << t->data << " "; 맨 마지막
		list* l = append(l1, l2); 
		return append(l, new list({ t->data,NULL }));// l 끝에다가 t->data
	}
}
...
list* inorder(tree* t) {
	if (t == NULL) {
		...
		return append(l1, new list({ t->data, l2 }));
		// return // l1 다음에 new list({t->data, l2})
	}
}
